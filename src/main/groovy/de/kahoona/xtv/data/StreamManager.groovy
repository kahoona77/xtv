 ...
 *
 * The maximum Downstream kilobits per sec speed can be set via setDownstreamKbps(long)}.
 * The maximum Upstream kilobits per sec speed can be set via {@link StreamManager#setUpstreamKbps(long)}.
 * Additional latency can be added via {@link StreamManager#setLatency(long)}.
 * It is possible to fine tune the amount of payload that gets actually sent to the pipe
 * via {@link StreamManager#setPayloadPercentage}.
 *
 ...
 * {@link StreamManager#registerStream(InputStream)}
 * {@link StreamManager#registerStream(OutputStream)}
 ...
 *
 */
class StreamManager implements BandwidthLimiter {
  ...
  private class StreamParams {
    ...
    private void adjustBytes(long bytesNumber) {
      ...
      remainingBps = Math.min(remainingBps + bytesNumber, (adjustedMaxBps / nextResetSubIntervals) as long );
    }
  }
  ...
  public StreamManager(long maxBitsPerSecond) {
    ...
    setDownstreamKbps(maxBitsPerSecond / 1000 as long);
    setUpstreamKbps(maxBitsPerSecond / 1000 as long);
   ...
   */
  public InputStream registerStream(InputStream is) {
    return new ManagedInputStream(is, this);
  }
